...
class ODE_TestCase9_Validate_Spatial_Values_in_VehicleSubscription(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config['VALIDATION_FILE'] = os.path.join('.', 'test_validation',
                                                      'TFR9_sub_veh_10_records_bad_Lat_Long_results.json')
    ...
    def run_validation_test(self):
        ...
        self.assertEquals(veh_count, len(valid_ode_output),
                          msg="Did not receive {} valid vehicle records".format(len(valid_ode_output)))
        self.assertEquals(total_records_received, len(valid_ode_output),
                          msg="Received {} more record(s) than expected ".format(
