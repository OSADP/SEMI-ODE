...
public class ITIScodesAndText extends SequenceOf {
	...
	public static class Item extends Choice {
		...
		case itis_chosen:
		    // Encode alternative 'itis'
		    try {
			ITIScodes item1 = (ITIScodes)this.mChosenValue;
		...
		case text_chosen:
		    // Encode alternative 'text'
		    try {
			ITIStext item1 = (ITIStext)this.mChosenValue;
		...
		switch (t_tag0) {
			...
			try {
			    ...
			    // Decode alternative 'text'
			    item1 = new ITIStext(coder.decodeString(source));
			    this.mChosenValue = item1;
     ...
     * This method is reserved for internal use and must not be invoked from the application code.
     */
    public ITIScodesAndText decodeValue(JsonCoder coder, JsonReader source)
	throws IOException, DecoderException
{
    int total_len0 = 0;
    int idx0 = 0;
