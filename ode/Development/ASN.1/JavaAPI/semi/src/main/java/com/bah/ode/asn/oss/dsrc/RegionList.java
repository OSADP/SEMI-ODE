...
public class RegionList extends SequenceOf {
	...
	if (total_len0 > 0) {
	    while (true) {
		try {
		    RegionOffsets item1 = this.elements.get(idx0);
     ...
     * This method is reserved for internal use and must not be invoked from the application code.
     */
    public RegionList decodeValue(JsonCoder coder, JsonReader source)
	throws IOException, DecoderException
{
	...
	this.elements = new java.util.ArrayList<RegionOffsets>(total_len0);
	...
	do {
		...
		DecoderException de = DecoderException.wrapException(e);
		de.appendElementContext(null, "RegionOffsets", idx0);
		throw de;
	    }
	    ++idx0;
