...
public class IntersectionRecord extends Sequence {
	...
	// Encode field 'mapData'
	try {
	    com.bah.ode.asn.oss.dsrc.MapData item1 = this.mapData;

	    {
		sink.encodeKey("mapData");
	...
	// Encode field 'spatData'
	try {
	    SpatRecord item1 = this.spatData;

	    {
		sink.writeSeparator();
		sink.encodeKey("spatData");
		item1.encodeValue(coder, sink);
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
     ...
     * This method is reserved for internal use and must not be invoked from the application code.
     */
    public IntersectionRecord decodeValue(JsonCoder coder, JsonReader source)
	...
	if (coder.hasMoreProperties(source, true))
		...
		IntersectionRecord.__Tag t_tag0 = IntersectionRecord.__Tag.getTagSub(tag0);
		if (t_tag0 == null) 
		    t_tag0 = IntersectionRecord.__Tag._null_;
		switch (t_tag0) {
		    case __mapData:
		    // Decode field 'mapData'
		    try {
			...
			if (this.mapData == null)
			    this.mapData = new com.bah.ode.asn.oss.dsrc.MapData();
			this.mapData.decodeValue(coder, source);
			present0[0] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("mapData", "MapData");
			...
			if (present0[1])
			    throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			if (this.spatData == null)
			    this.spatData = new SpatRecord();
			this.spatData.decodeValue(coder, source);
			present0[1] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("spatData", "SpatRecord");
	...
	if (!present0[0])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'mapData'");
	if (!present0[1])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'spatData'");
	return this;
    }
