...
public class ConnectionPoint extends Sequence {
	...
	// Encode field 'address'
	try {
	    IpAddress item1 = this.address;

	    if (item1 != null) {
		{
		    separator0 = ",";
		    sink.encodeKey("address");
		    item1.encodeValue(coder, sink);
		}
	    } else if (coder.isEncodingOfAbsentComponentsEnabled()) {
		separator0 = ",";
		coder.encodeAbsentComponent(sink, "address");
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("address", "IpAddress");
     ...
     * This method is reserved for internal use and must not be invoked from the application code.
     */
    public ConnectionPoint decodeValue(JsonCoder coder, JsonReader source)
	...
	if (coder.hasMoreProperties(source, true))
		...
		ConnectionPoint.__Tag t_tag0 = ConnectionPoint.__Tag.getTagSub(tag0);
		if (t_tag0 == null) 
		    t_tag0 = ConnectionPoint.__Tag._null_;
		switch (t_tag0) {
		    case __address:
		    // Decode field 'address'
		    try {
			if (!coder.isNullValue(source)) {
				...
				this.address = new IpAddress();
			    this.address.decodeValue(coder, source);
			    present0[0] = true;
			}
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			...
			if (present0[1])
			    throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			if (this.port == null)
			    this.port = new PortNumber();
			this.port.setValue(coder.decodeInteger(source));
			present0[1] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("port", "PortNumber");
	...
	if (!present0[0])
	    this.address = null;
	if (!present0[1])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'port'");
	return this;
    }
