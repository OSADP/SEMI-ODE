...
public class IntersectionSituationData extends Sequence implements BEREncodable, BERDecodable, DEREncodable, JSONEncodable, JSONDecodable, PEREncodable, PERDecodable {
	...
	__intersectionRecord("intersectionRecord"),
	_null_("_null_");
	private String tag;
	...
	// Encode field 'intersectionRecord'
	try {
	    IntersectionRecord item1 = this.intersectionRecord;

	    {
		sink.writeSeparator();
		sink.encodeKey("intersectionRecord");
		item1.encodeValue(coder, sink);
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
     ...
     * This method is reserved for internal use and must not be invoked from the application code.
     */
    public IntersectionSituationData decodeValue(JsonCoder coder, JsonReader source)
	...
	if (coder.hasMoreProperties(source, true))
		...
		IntersectionSituationData.__Tag t_tag0 = IntersectionSituationData.__Tag.getTagSub(tag0);
		if (t_tag0 == null) 
		    t_tag0 = IntersectionSituationData.__Tag._null_;
		switch (t_tag0) {
		    case __dialogID:
		    // Decode field 'dialogID'
		    try {
		    ...
		    break;
		    case __intersectionRecord:
		    // Decode field 'intersectionRecord'
		    try {
			if (present0[7])
			    throw new DecoderException(ExceptionDescriptor._field_repeat, null);
			if (this.intersectionRecord == null)
			    this.intersectionRecord = new IntersectionRecord();
			this.intersectionRecord.decodeValue(coder, source);
			present0[7] = true;
		    } catch (Exception e) {
	...
	if (!present0[7])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'intersectionRecord'");
	return this;
    }
