...
public class BasicSafetyMessageVerbose extends Sequence implements BEREncodable, BERDecodable, DEREncodable, JSONEncodable, JSONDecodable, PEREncodable, PERDecodable {
	...
	private static java.util.HashMap<String, __Tag> map =
	    new java.util.HashMap<String, __Tag>(17);
     ...
     * This method is reserved for internal use and must not be invoked from the application code.
     */
    public BasicSafetyMessageVerbose decodeValue(JsonCoder coder, JsonReader source)
	    throws IOException, DecoderException
    {
	boolean[] present0 = new boolean[17];

	coder.decodeObject(source);
	if (coder.hasMoreProperties(source, true))
	    do {
		String tag0 = coder.nextProperty(source);
		BasicSafetyMessageVerbose.__Tag t_tag0 = BasicSafetyMessageVerbose.__Tag.getTagSub(tag0);
		if (t_tag0 == null) 
		    t_tag0 = BasicSafetyMessageVerbose.__Tag._null_;
		switch (t_tag0) {
		    case __msgID:
		    // Decode field 'msgID'
		    try {
	...
	if (!present0[3])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'secMark'");
	if (!present0[4])
	...
	if (!present0[6])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'elev'");
	if (!present0[7])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'accuracy'");
	if (!present0[8])
	...
	if (!present0[13])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'size'");
	if (!present0[14])
	    this.safetyExt = null;
	if (!present0[15])
	    this.status = null;
