...
public class SnapshotTime extends Sequence {
	...
	if (coder.hasMoreProperties(source, true))
		...
		SnapshotTime.__Tag t_tag0 = SnapshotTime.__Tag.getTagSub(tag0);
		if (t_tag0 == null) 
		    t_tag0 = SnapshotTime.__Tag._null_;
		switch (t_tag0) {
		    case __t1:
		    // Decode field 't1'
		    try {
			...
			if (this.t1 == null)
			    this.t1 = new INTEGER();
			this.t1.setValue(coder.decodeInteger(source));
			present0[0] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("t1", "INTEGER");
			...
			if (this.t2 == null)
			    this.t2 = new INTEGER();
			this.t2.setValue(coder.decodeInteger(source));
			present0[2] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("t2", "INTEGER");
	...
	if (!present0[0])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 't1'");
	if (!present0[1])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 's1'");
	if (!present0[2])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 't2'");
	if (!present0[3])
