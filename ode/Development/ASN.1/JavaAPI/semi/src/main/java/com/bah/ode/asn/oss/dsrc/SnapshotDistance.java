...
public class SnapshotDistance extends Sequence {
	...
	if (coder.hasMoreProperties(source, true))
		...
		SnapshotDistance.__Tag t_tag0 = SnapshotDistance.__Tag.getTagSub(tag0);
		if (t_tag0 == null) 
		    t_tag0 = SnapshotDistance.__Tag._null_;
		switch (t_tag0) {
		    case __d1:
		    // Decode field 'd1'
		    try {
			...
			if (this.d1 == null)
			    this.d1 = new INTEGER();
			this.d1.setValue(coder.decodeInteger(source));
			present0[0] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("d1", "INTEGER");
			...
			if (this.s1 == null)
			    this.s1 = new INTEGER();
			this.s1.setValue(coder.decodeInteger(source));
			present0[1] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("s1", "INTEGER");
			...
			if (this.d2 == null)
			    this.d2 = new INTEGER();
			this.d2.setValue(coder.decodeInteger(source));
			present0[2] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("d2", "INTEGER");
			...
			if (this.s2 == null)
			    this.s2 = new INTEGER();
			this.s2.setValue(coder.decodeInteger(source));
			present0[3] = true;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendFieldContext("s2", "INTEGER");
	...
	if (!present0[0])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'd1'");
	if (!present0[1])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 's1'");
	if (!present0[2])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'd2'");
	if (!present0[3])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 's2'");
	return this;
    }
