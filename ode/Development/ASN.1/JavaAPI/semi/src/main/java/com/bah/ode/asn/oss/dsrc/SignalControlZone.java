...
public class SignalControlZone extends Sequence {
	...
	public static class LaneSet extends SequenceOf {
		...
		if (total_len0 > 0) {
		    while (true) {
			try {
			    LaneNumber item1 = this.elements.get(idx0);
	     ...
	     * This method is reserved for internal use and must not be invoked from the application code.
	     */
	    public LaneSet decodeValue(JsonCoder coder, JsonReader source)
		throws IOException, DecoderException
	{
		...
		this.elements = new java.util.ArrayList<LaneNumber>(total_len0);
		...
		do {
			...
			item1 = new LaneNumber(coder.decodeOctetString(source));
			this.elements.add(item1);
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendElementContext(null, "LaneNumber", idx0);
			throw de;
		    }
		    ++idx0;
	...
	public static class Zones extends SequenceOf {
		...
		public static class Enclosed extends SequenceOf {
		     ...
		     * This method is reserved for internal use and must not be invoked from the application code.
		     */
		    public Enclosed decodeValue(JsonCoder coder, JsonReader source)
			throws IOException, DecoderException
		{
		    int total_len0 = 0;
		    int idx0 = 0;
			...
			throws IOException, EncoderException, EncodeFailedException
		{
		    ...
		    sink.writeBit(false);
		    sink.writeBit(data.enclosed != null);
		...
		/** Decode root fields **/
			...
			try {
			    ...
			    data.enclosed.decodeValue(coder, source, data.enclosed);
			} catch (Exception e) {
			    DecoderException de = DecoderException.wrapException(e);
			...
			throws IOException, EncoderException
		{
		    ...
		    // Encode field 'enclosed'
		    try {
			Enclosed item1 = this.enclosed;

			if (item1 != null) {
			    {
				separator0 = ",";
				sink.encodeKey("enclosed");
				item1.encodeValue(coder, sink);
			    }
			} else if (coder.isEncodingOfAbsentComponentsEnabled()) {
			    separator0 = ",";
			    coder.encodeAbsentComponent(sink, "enclosed");
			}
		    } catch (Exception e) {
			EncoderException ee = EncoderException.wrapException(e);
			ee.appendFieldContext("enclosed", "SEQUENCE OF");
			...
			do {
				...
				try {
					...
					if (this.enclosed == null)
					    this.enclosed = new Enclosed();
					this.enclosed.decodeValue(coder, source);
					present0[0] = true;
				    }
				} catch (Exception e) {
				    DecoderException de = DecoderException.wrapException(e);
		...
		public int hashCode() {
		    int hash = 3;
		    hash = 41 * hash + (this.enclosed != null ? this.enclosed.hashCode() : 0);
	     ...
	     * This method is reserved for internal use and must not be invoked from the application code.
	     */
	    public Zones decodeValue(JsonCoder coder, JsonReader source)
		throws IOException, DecoderException
	{
	    int total_len0 = 0;
	    int idx0 = 0;
		...
		throws IOException, EncoderException
	{
		...
		// Encode alternative 'laneSet'
		try {
		    LaneSet item1 = (LaneSet)this.mChosenValue;
		...
		// Encode alternative 'zones'
		try {
		    Zones item1 = (Zones)this.mChosenValue;

		    sink.encodeKey("zones");
		    item1.encodeValue(coder, sink);
		} catch (Exception e) {
		    EncoderException ee = EncoderException.wrapException(e);
		    ee.appendElementContext("zones", "SEQUENCE OF", 0);
	 ...
	 * This method is reserved for internal use and must not be invoked from the application code.
	 */
	public Data decodeValue(JsonCoder coder, JsonReader source)
		throws IOException, DecoderException
	{
	    coder.decodeObject(source);
	    String tag0 = coder.nextProperty(source);
	    Data.__Tag t_tag0 = Data.__Tag.getTagSub(tag0);
	    if (t_tag0 == null) 
		t_tag0 = Data.__Tag._null_;
	    switch (t_tag0) {
		case __laneSet:
		    // Decode alternative 'laneSet'
		    try {
			...
			this.mChosenFlag = laneSet_chosen;
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
			de.appendElementContext("laneSet", "SEQUENCE OF", 0);
			throw de;
	...
	// Encode field 'pValue'
	try {
	    SignalReqScheme item1 = this.pValue;

	    {
		sink.writeSeparator(separator0);
		sink.encodeKey("pValue");
		sink.encodeOctetString(item1.byteArrayValue(), item1.getSize());
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	...
	if (coder.hasMoreProperties(source, true))
	    do {
		String tag0 = coder.nextProperty(source);
		SignalControlZone.__Tag t_tag0 = SignalControlZone.__Tag.getTagSub(tag0);
		if (t_tag0 == null) 
		    t_tag0 = SignalControlZone.__Tag._null_;
		switch (t_tag0) {
		    case __name:
		    // Decode field 'name'
		    try {
	...
	if (!present0[0])
	    this.name = null;
	if (!present0[1])
	    throw new DecoderException(ExceptionDescriptor._field_omit, ": 'pValue'");
	if (!present0[2])
	...
	hash = 41 * hash + (this.pValue != null ? this.pValue.hashCode() : 0);
