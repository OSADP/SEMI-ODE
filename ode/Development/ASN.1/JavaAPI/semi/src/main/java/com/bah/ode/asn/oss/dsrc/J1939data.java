...
public class J1939data extends Sequence {
		...
		if (coder.hasMoreProperties(source, true))
			...
			switch (t_tag0) {
				...
				if (!coder.isNullValue(source)) {
					...
					this.wheelEndElectFault = new WheelEndElectFault();
				    coder.decodeBitString(-1, source, this.wheelEndElectFault);
				    present0[4] = true;
				}
			    } catch (Exception e) {
				DecoderException de = DecoderException.wrapException(e);
	 ...
	 * This method is reserved for internal use and must not be invoked from the application code.
	 */
	public Tires decodeValue(JsonCoder coder, JsonReader source)
	    throws IOException, DecoderException
    {
	int total_len0 = 0;
	int idx0 = 0;
	 ...
	 * This method is reserved for internal use and must not be invoked from the application code.
	 */
	public Axle decodeValue(JsonCoder coder, JsonReader source)
	    throws IOException, DecoderException
    {
	int total_len0 = 0;
	int idx0 = 0;
	...
	try {
		...
		sink.writeSeparator(separator0);
		separator0 = ",";
		coder.encodeAbsentComponent(sink, "axle");
	    }
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("axle", "SEQUENCE OF");
	...
	if (coder.hasMoreProperties(source, true))
	    do {
		String tag0 = coder.nextProperty(source);
		J1939data.__Tag t_tag0 = J1939data.__Tag.getTagSub(tag0);
		if (t_tag0 == null) 
		    t_tag0 = J1939data.__Tag._null_;
		switch (t_tag0) {
		    case __tires:
		    // Decode field 'tires'
		    try {
			...
			if (!coder.isNullValue(source)) {
			    ...
			    if (this.axle == null)
				this.axle = new Axle();
			    this.axle.decodeValue(coder, source);
			    present0[1] = true;
			}
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
