...
public class EmergencyVehicleAlert extends Sequence implements BEREncodable, BERDecodable, DEREncodable, PEREncodable, PERDecodable {
	...
	boolean has_responseType0 = data.responseType != null;
	boolean has_details0 = data.details != null;
	boolean has_mass0 = data.mass != null;
	boolean has_basicType0 = data.basicType != null;
	boolean has_vehicleType0 = data.vehicleType != null;
	boolean has_responseEquip0 = data.responseEquip != null;
	boolean has_responderType0 = data.responderType != null;
	sink.writeBit(false); ++nbits;
	sink.writeBit(has_id0); ++nbits;
	sink.writeBit(has_responseType0); ++nbits;
	sink.writeBit(has_details0); ++nbits;
	sink.writeBit(has_mass0); ++nbits;
	sink.writeBit(has_basicType0); ++nbits;
	sink.writeBit(has_vehicleType0); ++nbits;
	sink.writeBit(has_responseEquip0); ++nbits;
	sink.writeBit(has_responderType0); ++nbits;
	// Encode field 'msgID'
	try {
    ...
    {
	try {
	    return EmergencyVehicleAlert.encodeValue(coder, sink, this);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext(null, "EmergencyVehicleAlert");
	    throw ee;
	...
	try {
	    ...
	    RoadSideAlert.decodeValue(coder, source, data.rsaMsg);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
    ...
    {
	try {
	    EmergencyVehicleAlert.decodeValue(coder, source, this);
	    return this;
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
