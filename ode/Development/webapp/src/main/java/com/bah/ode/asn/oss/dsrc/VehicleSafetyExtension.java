...
public class VehicleSafetyExtension extends Sequence {
	...
	boolean has_events0 = data.events != null;
	boolean has_pathHistory0 = data.pathHistory != null;
	boolean has_pathPrediction0 = data.pathPrediction != null;
	boolean has_theRTCM0 = data.theRTCM != null;
	sink.writeBit(false); ++nbits;
	sink.writeBit(has_events0); ++nbits;
	sink.writeBit(has_pathHistory0); ++nbits;
	sink.writeBit(has_pathPrediction0); ++nbits;
	sink.writeBit(has_theRTCM0); ++nbits;
	if (has_events0) {
	    // Encode field 'events'
	    try {
	...
	if (has_pathHistory0) {
		...
		PathHistory.decodeValue(coder, source, data.pathHistory);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
	...
	if (has_pathPrediction0) {
		...
		PathPrediction.decodeValue(coder, source, data.pathPrediction);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
	...
	if (has_theRTCM0) {
		...
		RTCMPackage.decodeValue(coder, source, data.theRTCM);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
