...
public class SignalStatusMessage extends Sequence implements BEREncodable, BERDecodable, DEREncodable, PEREncodable, PERDecodable {
	...
	boolean has_priorityCause0 = data.priorityCause != null;
	boolean has_prempt0 = data.prempt != null;
	boolean has_preemptCause0 = data.preemptCause != null;
	boolean has_transitStatus0 = data.transitStatus != null;
	sink.writeBit(false); ++nbits;
	sink.writeBit(has_priority0); ++nbits;
	sink.writeBit(has_priorityCause0); ++nbits;
	sink.writeBit(has_prempt0); ++nbits;
	sink.writeBit(has_preemptCause0); ++nbits;
    ...
    {
	try {
	    return SignalStatusMessage.encodeValue(coder, sink, this);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext(null, "SignalStatusMessage");
	    throw ee;
	...
	if (has_priority0) {
		...
		Priority.decodeValue(coder, source, data.priority);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
	...
	if (has_priorityCause0) {
		...
		VehicleIdent.decodeValue(coder, source, data.priorityCause);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
	...
	if (has_prempt0) {
		...
		Prempt.decodeValue(coder, source, data.prempt);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
	...
	if (has_preemptCause0) {
		...
		VehicleIdent.decodeValue(coder, source, data.preemptCause);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
    ...
    {
	try {
	    SignalStatusMessage.decodeValue(coder, source, this);
	    return this;
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
