...
public class OssReaderJsonWriter {
   ...
   private static Logger logger = Logger.getLogger(OssReaderJsonWriter.class
         .getName());
   ...
   public static void main(String args[]) {
      ...
      try {
         ...
         java.io.InputStream ins = buffering ? new java.io.BufferedInputStream(
         ...
         PrintWriter jsonOut = new PrintWriter(new PrintStream(filename
               + ".json"));
         PrintWriter printOut = new PrintWriter(new PrintStream(filename
               + ".print"));
         ...
         do {
            try {
               VehSitDataMessage value = (VehSitDataMessage) coder.decode(ins,
               ...
               for (int i = 0; i < bSize; i++) {
                  ...
                  OdeDateTime dateTime = new OdeDateTime(time.getYear()
                        .intValue(), time.getMonth().intValue(), time.getDay()
                        .intValue(), time.getHour().intValue(), time
                        .getMinute().intValue(), time.getSecond().intValue());

                  Position3D pos = vsr.getPos();
                  OdePosition3D location = new OdePosition3D(pos.getLat()
                        .longValue(), pos.get_long().longValue(), pos
                        .getElevation().byteArrayValue());

                  FundamentalSituationalStatus fund = vsr.getFundamental();

                  OdeVehicleSize size = new OdeVehicleSize(fund.getVehSize()
                        .getWidth().intValue(), fund.getVehSize().getLength()
                        .intValue());
