...
public class IntersectionRecord extends Sequence {
	...
	try {
	    ...
	    nbits += (com.bah.ode.asn.oss.dsrc.MapData.encodeValue(coder, sink, item1));
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext("mapData", "MapData");
	    throw ee;
	...
	try {
	    ...
	    com.bah.ode.asn.oss.dsrc.MapData.decodeValue(coder, source, data.mapData);
	} catch (Exception e) {
	...
	try {
	    ...
	    SpatRecord.decodeValue(coder, source, data.spatData);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
