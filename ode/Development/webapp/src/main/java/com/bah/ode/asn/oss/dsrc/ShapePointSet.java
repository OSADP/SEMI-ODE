...
public class ShapePointSet extends Sequence {
	...
	boolean has_anchor0 = data.anchor != null;
	boolean has_laneWidth0 = data.laneWidth != null;
	boolean has_directionality0 = data.directionality != null;
	sink.writeBit(false); ++nbits;
	sink.writeBit(has_anchor0); ++nbits;
	sink.writeBit(has_laneWidth0); ++nbits;
	sink.writeBit(has_directionality0); ++nbits;
	if (has_anchor0) {
	    // Encode field 'anchor'
	    try {
	...
	if (has_anchor0) {
		...
		Position3D.decodeValue(coder, source, data.anchor);
	    } catch (Exception e) {
