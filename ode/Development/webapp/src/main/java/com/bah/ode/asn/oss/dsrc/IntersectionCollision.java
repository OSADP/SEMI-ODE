...
public class IntersectionCollision extends Sequence implements BEREncodable, BERDecodable, DEREncodable, PEREncodable, PERDecodable {
	...
	boolean has_secMark0 = data.secMark != null;
	sink.writeBit(false); ++nbits;
	sink.writeBit(has_secMark0); ++nbits;
	// Encode field 'msgID'
	try {
    ...
    {
	try {
	    return IntersectionCollision.encodeValue(coder, sink, this);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext(null, "IntersectionCollision");
	    throw ee;
	...
	try {
	    ...
	    PathHistory.decodeValue(coder, source, data.path);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
    ...
    {
	try {
	    IntersectionCollision.decodeValue(coder, source, this);
	    return this;
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
