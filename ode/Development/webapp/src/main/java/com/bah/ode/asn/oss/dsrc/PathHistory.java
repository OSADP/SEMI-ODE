...
public class PathHistory extends Sequence {
	...
	public static class PathHistoryPointSets_01 extends SequenceOf {
		...
		while (idx0 > 0) {
			...
			PathHistoryPointType_01.decodeValue(coder, source, item1);
		    } catch (Exception e) {
		...
		throws IOException, DecoderException, DecodeFailedException
	{
		...
		case pathHistoryPointSets_01_chosen:
			...
			PathHistoryPointSets_01.decodeValue(coder, source, item1);
		    } catch (Exception e) {
			DecoderException de = DecoderException.wrapException(e);
	...
	boolean has_initialPosition0 = data.initialPosition != null;
	boolean has_currGPSstatus0 = data.currGPSstatus != null;
	boolean has_itemCnt0 = data.itemCnt != null;
	sink.writeBit(false); ++nbits;
	sink.writeBit(has_initialPosition0); ++nbits;
	sink.writeBit(has_currGPSstatus0); ++nbits;
	sink.writeBit(has_itemCnt0); ++nbits;
	...
	if (has_initialPosition0) {
		...
		FullPositionVector.decodeValue(coder, source, data.initialPosition);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
	...
	try {
	    ...
	    CrumbData.decodeValue(coder, source, data.crumbData);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
