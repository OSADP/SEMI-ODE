...
public class ApproachObject extends Sequence {
	...
	boolean has_approach0 = data.approach != null;
	boolean has_egress0 = data.egress != null;
	sink.writeBit(false); ++nbits;
	sink.writeBit(has_refPoint0); ++nbits;
	sink.writeBit(has_laneWidth0); ++nbits;
	sink.writeBit(has_approach0); ++nbits;
	sink.writeBit(has_egress0); ++nbits;
	if (has_refPoint0) {
	...
	if (has_approach0) {
		...
		Approach.decodeValue(coder, source, data.approach);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
	...
	if (has_egress0) {
		...
		Approach.decodeValue(coder, source, data.egress);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
