...
import clientConfig
import commonUtils
from odeClient import client
from utils.commonUtils import build_request

file_date_format = datetime.datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
logFile = 'depositClient{}.log'.format(file_date_format)
logger = commonUtils.get_logger('PythonSDK.depositClient', logging.INFO, logFile)
    ...
    with open(filePath, ) as f:
        ...
        for line in f:
            try:
                ...
                depositRequest = {}
                depositRequest['dataSource'] = config['depositSource'];
                depositRequest['dataType'] = config['depositDataType'];
                depositRequest['encodeType'] = config['depositEncodeType']
                depositRequest['data'] = line;
                ws.send(json.dumps(depositRequest))
                count += 1
                batchCount += 1
                #if count % 100 == 0:
                #    logger.info("deposited %d records", count)
            except:
                logger.debug("LINE: %s", line)
                logger.exception("Error Depositing data: %s", line)

            if batchPeriod is not None and batchCount >= batchRecords:
                time.sleep(batchPeriod/1000.0) # uncomment out and  adjust to change how slow/fast data is deposited
                batchCount = 0

        result = True
    logger.info('*****deposited %d records to the ODE *****', count)
