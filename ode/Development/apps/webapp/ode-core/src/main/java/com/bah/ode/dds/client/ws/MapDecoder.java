...
public class MapDecoder extends DdsDecoder {

   private static final Logger logger = LoggerFactory
         .getLogger(MapDecoder.class);
   ...
   public DdsData decode(String message) throws DecodeException {
      ...
      if (ddsData.getControlMessage() == null) {
         ...
         try {
             ...
             *  Map Data and ISD has both SPat and Map. So we create an empty
             *  IntersectionSituationData object for the ddsData where we could
             *  populate only what's required.
             */
            IntersectionSituationData map = new IntersectionSituationData();
            map.setIntersectionRecord(new IntersectionRecord( 
                  isd.getIntersectionRecord().getMapData(), null));
            ddsData.setIsd(map);
         } catch (Exception e) {
