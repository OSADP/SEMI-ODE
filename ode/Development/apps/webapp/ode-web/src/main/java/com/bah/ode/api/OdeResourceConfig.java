   ...
   public OdeResourceConfig() {
      register(new AbstractBinder() {
         ...
         protected void configure() {
            bindFactory(LiferayWSFactory.class).to(LiferayWSClient.class).in(
                  Singleton.class);
            bindFactory(TokenAuthenticationServiceFactory.class).to(
                  TokenAuthenticationService.class).in(Singleton.class);
