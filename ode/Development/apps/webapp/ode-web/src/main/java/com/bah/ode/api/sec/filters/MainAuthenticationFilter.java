...
public class MainAuthenticationFilter implements ContainerRequestFilter {
   ...
   public void filter(ContainerRequestContext requestContext)
	  ...
	  if (auth.startsWith(BASIC)) // && requestContext.getUriInfo().getPath().contains("auth/login")  )
	  {
	  ...
	  {
		  abortRequest(requestContext, Response.Status.UNAUTHORIZED , "Error Decoding Authorization Header");  
	  }
   }
   
   /**
    * Check to see if Token is still Valid (i.e. has not expired) 
    * If token has not expired, check to see if token  is still active in the token repository
    * @param authenticationHeaderValue
    * @param crc
    */
   private void tokenAuthentication(String authenticationHeaderValue, ContainerRequestContext crc) 
   {
		...
		try {
			...
			abortRequest(crc, Response.Status.UNAUTHORIZED ,"Unable to Authenticate token. Error " + e.toString());
		}
   }
   
   private void basicAuthentication(String authenticationHederValue,ContainerRequestContext crc)
   {
	   String[] lap = SecurityService.decode(authenticationHederValue);
      
      // If login or password fail
      if (lap == null || lap.length != 2) {
         abortRequest(crc,Response.Status.UNAUTHORIZED,"Missing UserName or Password");
      }
      long userId;
      ...
      try {
         ...
         crc.setProperty("userId", new Long(userId));
    	  ...
    	  abortRequest(crc,Response.Status.UNAUTHORIZED, "Unable to Authenticate. Error " + e.toString());
      }
   }

   private void abortRequest(ContainerRequestContext crc, Response.Status status, String message)
	{
		OdeStatus msg  = new OdeStatus();
		msg.setCode(OdeStatus.Code.FAILURE).setMessage(message);
		
		crc.abortWith(Response.status(status)
