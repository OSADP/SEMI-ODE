...
inputTopic=spark.vehicle.transformer.input.topic
...
echo sudo /usr/hdp/current/spark-client/bin/spark-submit  \
  ...
  --files ./spark-transformer-metrics.properties \
  --class $className \
  --properties-file ./spark-conf.properties \
  --conf spark.metrics.conf=$SPARK_HOME/conf/spark-transformer-metrics.properties \
  --conf spark.run.ode.aggregator.in.vdp=true \
