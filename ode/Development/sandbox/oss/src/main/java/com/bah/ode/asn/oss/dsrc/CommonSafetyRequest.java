...
public class CommonSafetyRequest extends Sequence implements BEREncodable, BERDecodable, DEREncodable, PEREncodable, PERDecodable {
	...
	boolean has_msgCnt0 = data.msgCnt != null;
	boolean has_id0 = data.id != null;
	sink.writeBit(false); ++nbits;
	sink.writeBit(has_msgCnt0); ++nbits;
    ...
    {
	try {
	    return CommonSafetyRequest.encodeValue(coder, sink, this);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext(null, "CommonSafetyRequest");
	    throw ee;
	...
	try {
	    ...
	    Requests.decodeValue(coder, source, data.requests);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
    ...
    {
	try {
	    CommonSafetyRequest.decodeValue(coder, source, this);
	    return this;
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
