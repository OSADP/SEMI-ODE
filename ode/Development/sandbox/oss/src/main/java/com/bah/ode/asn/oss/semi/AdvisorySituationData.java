...
public class AdvisorySituationData extends Sequence implements BEREncodable, BERDecodable, DEREncodable, PEREncodable, PERDecodable {
	...
	boolean has_recordID0 = data.recordID != null;
	boolean has_timeToLive0 = data.timeToLive != null;
	sink.writeBit(has_recordID0); ++nbits;
	sink.writeBit(has_timeToLive0); ++nbits;
    ...
    {
	try {
	    return AdvisorySituationData.encodeValue(coder, sink, this);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext(null, "AdvisorySituationData");
	    throw ee;
	...
	try {
	    ...
	    AdvisoryDetails.decodeValue(coder, source, data.asdmDetails);
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
    ...
    {
	try {
	    AdvisorySituationData.decodeValue(coder, source, this);
	    return this;
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
