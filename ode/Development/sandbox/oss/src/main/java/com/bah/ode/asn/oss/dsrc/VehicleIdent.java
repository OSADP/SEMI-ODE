...
public class VehicleIdent extends Sequence {
	...
	boolean has_vin0 = data.vin != null;
	boolean has_ownerCode0 = data.ownerCode != null;
	boolean has_id0 = data.id != null;
	boolean has_vehicleType0 = data.vehicleType != null;
	boolean has_vehicleClass0 = data.vehicleClass != null;
	sink.writeBit(false); ++nbits;
	sink.writeBit(has_name0); ++nbits;
	sink.writeBit(has_vin0); ++nbits;
	sink.writeBit(has_ownerCode0); ++nbits;
	sink.writeBit(has_id0); ++nbits;
	sink.writeBit(has_vehicleType0); ++nbits;
	sink.writeBit(has_vehicleClass0); ++nbits;
	if (has_name0) {
	    // Encode field 'name'
	    try {
	...
	if (has_vehicleClass0) {
		...
		VehicleClass.decodeValue(coder, source, data.vehicleClass);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
