...
public class ServiceRequest extends Sequence implements BEREncodable, BERDecodable, DEREncodable, PEREncodable, PERDecodable {
	...
	boolean has_destination0 = data.destination != null;
	sink.writeBit(has_destination0); ++nbits;
    ...
    {
	try {
	    return ServiceRequest.encodeValue(coder, sink, this);
	} catch (Exception e) {
	    EncoderException ee = EncoderException.wrapException(e);
	    ee.appendFieldContext(null, "ServiceRequest");
	    throw ee;
	...
	if (has_destination0) {
		...
		ConnectionPoint.decodeValue(coder, source, data.destination);
	    } catch (Exception e) {
		DecoderException de = DecoderException.wrapException(e);
    ...
    {
	try {
	    ServiceRequest.decodeValue(coder, source, this);
	    return this;
	} catch (Exception e) {
	    DecoderException de = DecoderException.wrapException(e);
