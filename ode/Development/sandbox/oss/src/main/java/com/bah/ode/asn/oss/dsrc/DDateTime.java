...
public class DDateTime extends Sequence {
	...
	boolean has_year0 = data.year != null;
	boolean has_month0 = data.month != null;
	boolean has_day0 = data.day != null;
	boolean has_hour0 = data.hour != null;
	boolean has_minute0 = data.minute != null;
	boolean has_second0 = data.second != null;
	sink.writeBit(has_year0); ++nbits;
	sink.writeBit(has_month0); ++nbits;
	sink.writeBit(has_day0); ++nbits;
	sink.writeBit(has_hour0); ++nbits;
	sink.writeBit(has_minute0); ++nbits;
	sink.writeBit(has_second0); ++nbits;
	if (has_year0) {
	    // Encode field 'year'
	    try {
